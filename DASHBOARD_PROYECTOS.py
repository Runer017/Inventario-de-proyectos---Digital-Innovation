...
porcentaje_completos = (
    proyectos_completos / len(df_cerrados) * 100
...
st.markdown(
    "<h3 style='margin-bottom:0;'>Inventario de productos</h3>",
    unsafe_allow_html=True
)

st.markdown(
    "<p style='color:#777; margin-top:4px;'>"
    "Productos digitales asociados a las automatizaciones"
    "</p>",
    unsafe_allow_html=True
)
...
filtro_tipo, filtro_producto = st.columns(2)
...
with filtro_tipo:
    ...
    tipos_productos = sorted(
        productos["TIPO"]
        .dropna()
        .unique()
    )

    tipo_seleccionado_producto = st.selectbox(
        "Tipo",
        options=["Todos"] + list(tipos_productos),
        key="filtro_tipo_producto"
    )
...
with filtro_producto:

    productos_opciones = sorted(
        productos["PRODUCTO"]
        .dropna()
        .unique()
    )
    ...
    producto_seleccionado = st.selectbox(
        "Producto",
        options=["Todos"] + list(productos_opciones),
        key="filtro_producto"
    )
...
if tipo_seleccionado_producto != "Todos":

    productos_filtrados = productos_filtrados[
        productos_filtrados["TIPO"]
        == tipo_seleccionado_producto
    ]
...
st.markdown(
    ...
    <div style="
        ...
        margin-bottom:12px;
    ">
        Mostrando <b>{len(productos_filtrados)}</b>
        de <b>{len(productos)}</b> registros
    </div>
    """,
    unsafe_allow_html=True
)
...
tabla_productos = productos_filtrados[
    [
        "COD_PROYECTO",
        "COD_ROBOT",
        "EMPRESA",
...
st.dataframe(
    ...
    column_config={
        ...
        "COD_PROYECTO": st.column_config.TextColumn(
            "Código Proyecto",
            width="medium"
        ),

        "COD_ROBOT": st.column_config.TextColumn(
            "Código Robot",
            width="small"
        ),
